...
import { measureTourTarget, waitForTourTarget } from './tourTargetRegistry';
import {
  subscribeTourTargetActivation,
  tourTargetIdFromSelector,
} from './tourActivation';
import { stepPopoverVariant } from './stepPopoverVariant';
import { TOUR_POST_CLICK_SETTLE_MS, TOUR_SCROLL_SETTLE_MS, TOUR_STEP_PREPARE_MS, tourPause } from './tourTiming';
import type { HighlightRect, PopoverPosition, TourPhase, TourStep, UseAppTourResult } from './types';

const TARGET_POLL_MS = 120;
const TARGET_POLL_MAX = 40;

type GoToStepOptions = {
  afterTargetClick?: boolean;
};

function waitForDomTarget(selector: string): Promise<HighlightRect | null> {
  return new Promise((resolve) => {
    let attempts = 0;
    const tick = () => {
      const rect = measureSelector(selector);
      if (rect) {
        resolve(rect);
        return;
      }
...
async function measureTargetForStep(step: TourStep): Promise<HighlightRect | null> {
  if (!step.targetSelector) return null;
  const targetId = tourTargetIdFromSelector(step.targetSelector);

  if (typeof document !== 'undefined') {
    let rect = measureSelector(step.targetSelector);
    if (!rect) rect = await waitForDomTarget(step.targetSelector);
    if (rect) return rect;
  }

  if (targetId) {
    let rect = await measureTourTarget(targetId);
    if (!rect) rect = await waitForTourTarget(targetId);
    return rect;
  }
...
export function useAppTour(steps: TourStep[]): UseAppTourResult {
  ...
  const syncLayoutForStep = useCallback(async (step: TourStep | null) => {
    ...
    if (!step?.targetSelector || variant === 'hero') {
      setTargetRect(null);
      setPopoverPosition(computePopoverPosition(null, step?.placement ?? 'auto', 0, variant));
      return;
    }

    scrollTargetIntoView(step.targetSelector);
    await tourPause(SPOTLIGHT_TRANSITION_MS / 2);

    if (step.scrollIntoView && typeof document !== 'undefined') {
      await tourPause(TOUR_SCROLL_SETTLE_MS);
    }

    const rect = await measureTargetForStep(step);
    setTargetRect(rect);
    setPopoverPosition(
      computePopoverPosition(rect, step.placement, step.spotlightPadding ?? 8, variant)
    );
  }, []);
      ...
      if (options?.afterTargetClick) {
        ...
        if (prevStep?.afterClick) {
          try {
            ...
            console.warn('[AppTour] afterClick failed', prevStep.id, e);
          }
        }
        ...
        try {
          ...
          console.warn('[AppTour] prepare step failed', step.id, e);
        }
  ...
  useEffect(() => {
    ...
    const remeasure = () => {
      ...
      const variant = stepPopoverVariant(step);
      if (!step.targetSelector || variant === 'hero') {
        setPopoverPosition(computePopoverPosition(null, step.placement, 0, variant));
        return;
      }
      void measureTargetForStep(step).then((rect) => {
        setTargetRect(rect);
        setPopoverPosition(
          computePopoverPosition(rect, step.placement, step.spotlightPadding ?? 8, variant)
        );
